...
export default function Verification() {
  ...
  const totalDonations = currentUser.donationsReceived.filter(donation=>donation.donationStatus === 'Complete').length;
  const totalMonetaryDonations = currentUser.donationsReceived.filter(donation=>donation.donationCategory === 'Monetary' && donation.donationStatus === 'Complete').length;
  const totalFoodDonations = currentUser.donationsReceived.filter(donation=>donation.donationCategory === 'Food' && donation.donationStatus === 'Complete').length;
  const totalRationDonations = currentUser.donationsReceived.filter(donation=>donation.donationCategory === 'Ration' && donation.donationStatus === 'Complete').length;
  const totalClothesDonations = currentUser.donationsReceived.filter(donation=>donation.donationCategory === 'Clothes' && donation.donationStatus === 'Complete').length;
  const totalMedicineDonations = currentUser.donationsReceived.filter(donation=>donation.donationCategory === 'Medicine' && donation.donationStatus === 'Complete').length;
  ...
  const totalMoney = currentUser.donationsReceived.filter(donation=>donation.donationCategory === 'Monetary' && donation.donationStatus === 'Complete');
  const totalOneTimeMoney = currentUser.donationsReceived.filter(donation=>donation.donationCategory === 'Monetary' && donation.donationStatus === 'Complete' && donation.donation.type === 'One Time');
  const totalSubscriptionMoney = currentUser.donationsReceived.filter(donation=>donation.donationCategory === 'Monetary' && donation.donationStatus === 'Complete' && donation.donation.type === 'Subscription');
    ...
    return (
      <ScrollView style={styles.container}>
        <View style={{overflow:"hidden",}}>
          <Text style={styles.title}>Donations Recieved ({totalDonations})</Text>
        ...
        <View style={{overflow:"hidden",}}>
          <Text style={styles.title}>Monetary Rs.({sumAll})</Text>
          <View style={{flexDirection:"row", marginBottom:16, alignItems:"center"}}>
